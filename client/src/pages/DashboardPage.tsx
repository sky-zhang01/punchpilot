...
function calcWorkMinutes(times: PunchTime[]): number {
  ...
  for (const { type, time } of times) {
    if (type === "checkin" || type === "break_end") {
      workStart = time;
    } else if ((type === "break_start" || type === "checkout") && workStart) {
      const [ah, am] = workStart.split(":").map(Number);
...
const DashboardPage: React.FC = () => {
  ...
  const workMinutes = calcWorkMinutes(punchTimesRaw);
    ...
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
          ...
          <Space direction="vertical" size="middle" style={{ width: "100%" }}>
            ...
            <Row gutter={[16, 16]} align="middle">
              ...
              <Col>
                <Tag color={stateColor} style={{ fontWeight: 600 }}>
                  {t(
                    STATE_LABEL_KEYS[detectedState] ||
                      "manualTrigger.stateUnknown",
                  )}
                </Tag>
              </Col>
            </Row>
            ...
            {statusData.startup_analysis?.state && (
              <Text type="secondary" style={{ display: "block" }}>
                {t(
                  STATE_REASON_MAP[statusData.startup_analysis.state] ||
                    "analysis.unknown",
                )}
                ...
                message={
                  statusData.next_action.mode === "random"
                    ? t("dashboard.nextActionRandom", {
                        action: t(
                          `actions.${snakeToCamel(statusData.next_action.action_type || "")}`,
                        ),
            ...
            {statusData.today_logs && (
              <Row gutter={16}>
                ...
                <Col span={8}>
                  <div>
                    <Text type="secondary" style={{ fontSize: 14 }}>
                      {t("dashboard.punchCount")}
                    </Text>
                    <div style={{ fontSize: 24, fontWeight: 600 }}>
                      {punchTimesRaw.length}
                    </div>
                  </div>
                </Col>
                <Col span={8}>
      ...
      {statusData &&
        (() => {
          ...
          if (!hasPunchData) {
            if (["working", "on_break", "checked_out"].includes(detectedState))
              completedByState.add("checkin");
            if (["on_break", "checked_out"].includes(detectedState))
              completedByState.add("break_start");
            if (detectedState === "checked_out") {
              completedByState.add("break_end");
              completedByState.add("checkout");
            }
            if (logMap["checkout"]?.status === "success") {
              completedByState.add("checkin");
              completedByState.add("break_start");
              completedByState.add("break_end");
            }
            if (logMap["break_end"]?.status === "success") {
              completedByState.add("checkin");
              completedByState.add("break_start");
            }
            if (logMap["break_start"]?.status === "success") {
              completedByState.add("checkin");
            }
          }
          ...
          const isStepDone = (step: string, i: number) => {
            ...
            return (
              logMap[step]?.status === "success" || completedByState.has(step)
            );
          };

          // Check if ANY step has actual evidence of completion (log/state)
          // If so, remaining skips are stale (user intervened manually)
          const hasAnyEvidence = punchSteps.some(
            (s) => logMap[s]?.status === "success" || completedByState.has(s),
          );
            ...
            >
              <Steps
                ...
                items={punchSteps.map((step, i) => {
                  const done = isStepDone(step, i);
                  const freeTime =
                    hasPunchData && i < punchTimes.length
                      ? punchTimes[i].time
                      : undefined;
                  const log = logMap[step];
                  // Only show skip icon when ALL steps were skipped with no manual intervention
                  const isSkipped =
                    allSkippedNoEvidence && !done && skippedSet.has(step);

                  // Step label: append number when the same type appears more than once
                  const sameTypeBefore = punchSteps
                    .slice(0, i)
                    .filter((s) => s === step).length;
                  const sameTypeTotal = punchSteps.filter(
                    (s) => s === step,
                  ).length;
                  const needsNumber =
                    sameTypeTotal > 1 &&
                    ["break_start", "break_end"].includes(step);
                  const title =
                    t(`actions.${snakeToCamel(step)}`) +
                    (needsNumber ? ` ${sameTypeBefore + 1}` : "");

                  let status: "wait" | "process" | "finish" | "error" = done
                    ? "finish"
                    : "wait";
      ...
      <Card
        title={
          <Space>
            ...
            {(() => {
              const companyName = statusData?.today_logs?.find(
                (l: LogEntry) => l.company_name,
              )?.company_name;
              return companyName ? (
